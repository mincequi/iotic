...
OnOffRule::OnOffRule(const std::string& thingId, ExprPtr&& onExpression, ExprPtr&& offExpression, Action action) :
    Rule(thingId),
    ...
    _subject.get_observable()
            ...
            .debounce(std::chrono::milliseconds(cfg->valueOr<int>(thingId, Config::Key::debounce, 60000)), rpp::schedulers::new_thread{})
            .observe_on(rppqt::schedulers::main_thread_scheduler{})
            .subscribe([this](bool isOn) {
        LOG_S(INFO) << this->thingId() << "> " << isOn;
        _action(isOn);
    });
}
