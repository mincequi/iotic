...
RulesEngine::RulesEngine(const ThingsRepository& thingsRepository) :
    ...
    primarySymbolTable.create_variable(util::toString(ReadableThingProperty::pv_power));
    primarySymbolTable.create_variable(util::toString(ReadableThingProperty::grid_power));
    primarySymbolTable.create_variable(util::toString(ReadableThingProperty::site_power));

    _thingsRepository.site().properties().subscribe([](const auto& prop) {
        for (const auto& kv : prop) {
            switch (kv.first) {
            case ReadableThingProperty::pv_power:
            case ReadableThingProperty::grid_power:
            case ReadableThingProperty::site_power: {
