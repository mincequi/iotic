...
MultiPhaseStrategy::MultiPhaseStrategy(
    ...
    thing->propertiesObservable().subscribe([&](const ThingPropertyMap& map) {
        map.on<ThingPropertyKey::digitalInput>([&](const auto& value) {
            bool allTrue = std::all_of(value.begin(), value.end(), [](bool v) { return v; });
            if (_isInputActive && !allTrue) {
                decrementPhaseLimit();
            }
            _isInputActive = allTrue;
...
bool MultiPhaseStrategy::wantsToStepDown(const Site::Properties& siteProperties) const {
    ...
    // If digital input is not active, we want to step down
    if (!_isInputActive) {
        return true;
    }
...
void MultiPhaseStrategy::adjust(Step step, const Site::Properties& siteProperties) {
    ...
    case Step::Keep:
        break;
    case Step::Down:
        _deliveredEnergy += (siteProperties.ts - _timestamps[_nextStepDownPhase]) * _powerThresholds[_nextStepDownPhase];
    ...
    for (size_t i = 0; i < _currentStates.size(); ++i) {
        if (_currentStates[i]) {
            currentPower += _powerThresholds[i];
        }
    }
    properties.set<ThingPropertyKey::power>(currentPower);

    // Energy
    int currentEnergy = 0;
    for (size_t i = 0; i < _currentStates.size(); ++i) {
        if (_currentStates[i]) {
            currentEnergy += (siteProperties.ts - _timestamps[i]) * _powerThresholds[i];
        }
    }
    // Convert Ws into kWm
    properties.set<ThingPropertyKey::energy>((_deliveredEnergy + currentEnergy) / 60000);

    // Set properties
    _thing->setProperties(properties);

    // Log phases if changed
    if (step != Step::Keep) {
        std::stringstream ss;
        ss << "[ ";
...
void MultiPhaseStrategy::decrementPhaseLimit() const {
    if (_phaseLimit == 0) {
        return;
    }

    _phaseLimitTimepoint = std::chrono::system_clock::now();
    _phaseLimit = std::max(_phaseLimit - 1, 0);
    LOG_S(INFO) << "decrement phase limit to " << _phaseLimit;
}

void MultiPhaseStrategy::incrementPhaseLimit() const {
    if (_phaseLimit >= (int)_powerThresholds.size()) {
        return;
    }

    // Return, if we decremented phases in the last 90.
    auto now = std::chrono::system_clock::now();
    if (now - _phaseLimitTimepoint < _configRepository.phaseLimitDuration()) {
        return;
    }

    _phaseLimitTimepoint = now;
