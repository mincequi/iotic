...
PowerManager::PowerManager(const StrategyRepository& strategyRepository,
    ...
    site.properties().subscribe([&](const Site::Properties& siteProperties) {
        ...
        for (auto it = strategies.begin(); it != strategies.end(); ++it) {
            ...
            if (strategy->wantsToStepDown(siteProperties) && siteProperties.ts >= _lastStepDownTs + ConfigRepository::stepDebounceSeconds) {
        ...
        for (auto it = strategies.rbegin(); it != strategies.rend(); ++it) {
            const auto& strategy = *it;
            if (strategy->wantsToStepUp(siteProperties) && siteProperties.ts >= _lastStepUpTs + ConfigRepository::stepDebounceSeconds) {
